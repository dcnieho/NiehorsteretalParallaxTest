...
\usepackage{datatool}	% for marker and target positions
...
\begin{document}
\newlength{\markerSizeAsLength}
\setlength{\markerSizeAsLength}{\markerSideCm}
\DTLloaddb{markerdata}{\markerPosFile}
\DTLloaddb{targetdata}{\targetPosFile}
\newcommand*{\markerSize}{}
\newcommand*{\getSize}[1]{%
	\DTLgetvalueforkey{\markerSize}{size}{markerdata}{ID}{#1}%
}
...
\DTLforeach{markerdata}{%
	\ID=ID, \x=x, \y=y, \rotang=rotation_angle%
}{%
	\coordinate (m) at (\x*\step, \y*\step);
	
	\DTLifhaskey{markerdata}{size}{
		\getSize{\ID}
		\pgfmathsetlength{\markerSizeAsLength}{\markerSize cm}
	}{}
	
	\node[inner sep=0pt] (m2) at (m) {\includegraphics[width=\markerSizeAsLength,angle=\rotang,origin=c]{\markerFilename{\ID}}};
}

\DTLforeach{targetdata}{%
