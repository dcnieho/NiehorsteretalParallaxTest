...
def prepare_background_targets(win: visual.Window, config: dict, run_id: int, placeholders: bool = True):
    ...
    fiducial_positions = read_coord_file(config["markers"]["files"][run_id])
    ...
    if replace_IDs is not None:
        replace_ID_start = config["markers"]["replace_ID_start"][run_id]+run_id*config["markers"]["replace_ID_offset"][run_id]
        for ri,m_id in enumerate(replace_IDs[run_id]):
            m_id_start = replace_ID_start+ri
            indicator_IDs[m_id] = {t_id: m_id_start+t_ii*len(replace_IDs[run_id]) for t_ii,t_id in enumerate(target_positions.index)}
...
def run_parallax(win: visual.Window, config: dict, refresh_rate: int, task_vars: dict):
    ...
    for t_id in targets:
        ...
        old_pos_pix = tools.monitorunittools.convertToPix(np.array([0.,0.]),np.astype(old_pos,'float64'),config["targets"]["units"],win)
        pos_pix     = tools.monitorunittools.convertToPix(np.array([0.,0.]),np.astype(    pos,'float64'),config["targets"]["units"],win)
...
def main():
    ...
    try:
        ...
        if 1/win.monitorFramePeriod<config["screen"]["refresh_rate"]-8 or 1/win.monitorFramePeriod>config["screen"]["refresh_rate"]+8:  # check within 8 Hz
        ...
        # do walking
        textstim.text = 'Now do the walking task.\n\n(Press the spacebar to continue when you are done walking)'
        textstim.draw()
        win.flip()
        event.waitKeys()
